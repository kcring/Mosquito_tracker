function opt_trm= refineExtrCalib(floc, frmloc)
...
addpath ./TOOLBOX_calib % for rodrigues function
addpath ./minimize_fex/ % for minimize function
if nargin < 1
...
for cam_ii=1:nc
    cam_id=[image_id, camid_suffixes(cam_ii)];
    disp(sprintf('Listing files... ls %s%s*.*',  frmloc, cam_id));
    c(cam_ii).flist = dir([frmloc, cam_id, '*.*']);
...
% At least 6 randomly chosen points must be selected from randomly chosen
% images. After that more points
total_points=8; % multiples of 6

% number of points clicked per image
npi=3;

% initial values ** this can be changed
rpts0=[50 50 300]; % the position of a point in mm
rpts_range=[300 300 300]*2; % range in mm
cr_range=[10,10,10]/10;
th_range=[pi/3 pi/3 pi/3]/10;
...
if(exist(uvfile, 'file'))
    load(uvfile);
    uv_jj=size(uv_match,1);
    uv_constr=uv_constr(1,:);
else
    uv_jj=0;
    uv_constr=zeros(2,9);
    ...
    for img_ii=img_ids'
        ...
        for cam_ii=1:nc
            ...
            imshow(img); hold on;
            title(c(cam_ii).flist(img_ii).name, 'interpreter', 'none');
        end
        
        proc=input('Image OK? Proceed? (zoom in if needed) ([]=yes, other=no):');
        if (isempty(proc))
            figure(1); gcf;
            title(sprintf('click on %d point pairs, left then right, in both images', npi));
            for uv_ii=1:npi
                uv_jj=uv_jj+1;
                for cam_ii=1:nc
                    ...
                    uv_match(uv_jj, 2*cam_ii-1)=uu;
                    uv_match(uv_jj, 2*cam_ii)=vv;
                end
            end
        end
    end
    ...
    for img_ii=img_ids(1)'
        ...
        if (isempty(proc))
            figure(1); gcf;
            title(sprintf('click point pairs left then right, corresponding to a fix dimension in both images'));
            for uv_ii=1:npi1
                uv_cc=uv_cc+1;
                for cam_ii=1:nc
                    subplot(1,nc,cam_ii); gca;
                    [uu, vv]=ginput(2);
                    plot(uu,vv, 'rs');
                    uv_constr(uv_cc, 4*(cam_ii-1)+1:4*(cam_ii-1)+2)=uu';
                    uv_constr(uv_cc, 4*(cam_ii-1)+3:4*(cam_ii-1)+4)=vv';
                end
                dimsize=input('what is the distance (in mm) between these two points? ');
                uv_constr(uv_cc, 4*nc+1)=dimsize;
            end
        end
    end
end
...
if(lsvar > 1)
    ...
    th_min=th0-th_range';
    cr_min=cr0-cr_range';
    rmin=r0-repmat(rpts_range', n_points,1);
    
    Xmin=[th_min; cr_min; rmin];
    
    th_max=th0+th_range';
    cr_max=cr0+cr_range';
    rmax=r0+repmat(rpts_range', n_points,1);
    ...
    [opt_val, fval, residual]=fmincon(@(X) optfun(X, uv_match, camstrs, tbf_camnum), ...
        X0, ...
        [],[],[],[],...
        Xmin,Xmax,...
        @(X) nonlcon(X, uv_constr, camstrs), ...
        options);
...
save(uvfile, 'uv_match', 'uv_constr');

% optimization function
function fval=optfun(X, uv_match, camstrs, tbf_camnum)

fval=0;

n_points=size(uv_match,1);
nc=size(uv_match,2)/2;
...
for np_ii=1:n_points
    for c_ii = 1:nc
        ...
        fval=norm(pval-uv_match(np_ii,2*c_ii-1:2*c_ii)')+fval;
    end
end
...
function [c, ceq]=nonlcon(X, uvconstr, cams)

c=0;
ceq=0;
opt_rot_matrix=rodrigues(X(1:3));

cams(2).trm=[opt_rot_matrix, X(4:6)
    0 0 0 1];

for jj=1:size(uvconstr,1)
    Uk1=[uvconstr(jj,[1 3])', uvconstr(jj,[5,7])'];
    Uk2=[uvconstr(jj,[2 4])', uvconstr(jj,[6,8])'];
    r1=lsTriangulate(Uk1, cams);
    r2=lsTriangulate(Uk2, cams);
    ceq=abs(norm(r1-r2)-uvconstr(jj,9))+ceq;
end
